...
def __encode__(to_encode, iv, key):
    ...
    # Fix: check key length
    key_bytes = key.encode('utf-8')
    if len(key_bytes) not in [16, 24, 32]:
        raise ValueError(f"Invalid AES key length: {len(key_bytes)} bytes! It must be 16, 24, or 32 bytes.")
    
    # Encrypt
    c = AES.new(key_bytes, AES.MODE_CBC, iv.encode('utf-8'))
    to_encode = c.encrypt(to_encode.encode('utf-8'))
    # Encode
    to_encode = base64.b64encode(to_encode)
    return to_encode.decode("UTF-8")
